...
// KOMPONEN ACCORDION RUTE (LENGKAP DENGAN SEMUA DETAIL KOLOM)
    ...
    <div className="accordion-item">
      <div className="accordion-header" onClick={() => setIsOpen(!isOpen)}>
        ...
        <span className="accordion-metrics">
          <span>
            Load: <strong style={{ color: '#38bdf8' }}>{rute.load}</strong>
          </span>
...
const PointAccordion = ({ point, badgeClass }) => {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div className="accordion-item">
      <div className="accordion-header" onClick={() => setIsOpen(!isOpen)}>
...
export default function App() {
  ...
  const filteredPoints =
    dashboardData?.pointRows?.filter((p) =>
      p.name.toLowerCase().includes(searchQuery.toLowerCase())
    ) || [];
  const filteredPinHistory = routePinPointHistoryData.filter(
    (item) =>
      item.routeCode.toLowerCase().includes(pinSearchQuery.toLowerCase()) ||
      item.pickupPointName.toLowerCase().includes(pinSearchQuery.toLowerCase())
  );
  ...
  const renderCalendar = () => {
    ...
    for (let i = 26; i <= 31; i++)
      days.push(
        <div key={`prev-${i}`} className="cal-cell-mini other-month">
          {i}
        </div>
      );
    for (let d = 1; d <= 31; d++) {
      ...
      days.push(
        <div
          key={d}
          className={`cal-cell-mini ${
            mode === 'daily' && dateStr === selectedDate ? 'selected' : ''
          }`}
          onClick={() => {
            setMode('daily');
    ...
    <>
      ...
      <div className={`sidebar ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        <div className="nav-menu">
          ...
          <div
            className={`nav-item ${activeMenu === 'routes' ? 'active' : ''}`}
            onClick={() => handleMenuClick('routes')}
          >
            🚚 Load & SLA
          </div>
          <div
            className={`nav-item ${activeMenu === 'points' ? 'active' : ''}`}
            onClick={() => handleMenuClick('points')}
          >
            📍 Info Point Task
          </div>
          <div
            className={`nav-item ${activeMenu === 'geotag' ? 'active' : ''}`}
            onClick={() => handleMenuClick('geotag')}
          >
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          >
            ...
            <button className={`mode-btn ${mode === 'daily' ? 'active' : ''}`}>
              📅 Harian
            </button>
          </div>
          <div className="sidebar-calendar">
            ...
            >
              {['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'].map((hari) => (
                <div key={hari} className="cal-day-lbl">
                  {hari}
                </div>
              ))}
              {renderCalendar()}
        ...
        <div className="dashboard-container">
          ...
          <div className="kpi-grid">
            <div className="kpi-card">
              <div className="title">Total Trip</div>
              <div className="value">
                {dashboardData?.kpi?.tripCount || '0'}
              </div>
              <div className="subtext">Unit Aktif</div>
            </div>
            <div className="kpi-card">
              <div className="title">Pure Pick-Up</div>
              <div className="value">
                {dashboardData?.kpi?.totalPurePU || '0'}
              </div>
              <div className="subtext">Pengambilan</div>
            </div>
            <div className="kpi-card">
              <div className="title">Pure Drop SS</div>
              <div className="value">
                {dashboardData?.kpi?.totalPureDrop || '0'}
              </div>
              <div className="subtext">Penurunan</div>
            </div>
            <div className="kpi-card">
              <div className="title">Total Workload</div>
              <div className="value">
                {dashboardData?.kpi?.totalWorkloadEffort || '0'}
              </div>
              <div className="subtext">Points Visit</div>
            </div>
            <div className="kpi-card">
              <div className="title">SLA On-Time</div>
              <div className="value">
                {dashboardData?.kpi?.overallSlaPct || '0'}%
              </div>
              <div className="subtext">Aman (No Delay)</div>
            </div>
            <div className="kpi-card">
              <div className="title">Real Load Factor</div>
              <div className="value">
                {dashboardData?.kpi?.overallLoadPct || '0'}%
              </div>
              <div className="subtext">Eq-PU Volume</div>
            </div>
          </div>
                ...
                <div className="mobile-accordion-list">
                  {filteredRoutes.map((r, i) => (
                    <RouteAccordion
                      key={i}
                      rute={r}
                      badgeClass={getBadgeClass(r.status)}
                    />
                  ))}
                </div>
              </div>
          ...
          {activeMenu === 'geotag' && (
            <div className="card-panel">
              ...
              >
                <h3>🗺️ History Pin Point Pickup per Kode Rute</h3>
                <input
                  type="text"
                  placeholder="Cari Kode Rute (e.g. STR-BTS59-STR)..."
              ...
              >
                ...
                >
                  {filteredPinHistory.map((item) => (
                    <div
                      key={item.routeCode}
                      onClick={() => setSelectedPinPoint(item)}
                      style={{
                        padding: '14px',
                        borderRadius: '10px',
                        cursor: 'pointer',
                        transition: 'all 0.2s',
                        border: `1px solid ${
                          selectedPinPoint.routeCode === item.routeCode
                            ? '#38bdf8'
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: '10px', color: 'var(--app-muted)' }}
                      >
                        🕒 Waktu: {item.timestamp}
                      </div>
                    </div>
                  ))}
                  {filteredPinHistory.length === 0 && (
                    <div
                      style={{
                        padding: '20px',
                        textAlign: 'center',
                    ...
                    >
                      Kode rute tidak ditemukan dalam riwayat pin point.
                    </div>
                  )}
                </div>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        🎯 RUTE: {selectedPinPoint.routeCode}
                      </span>
                      <div
                        style={{
                          fontSize: '10px',
                      ...
                      >
                        Driver: {selectedPinPoint.driver} • Muatan:{' '}
                        {selectedPinPoint.volume}
                      </div>
                    </div>
                    <span
                      style={{
                    ...
                    >
                      {selectedPinPoint.lat}, {selectedPinPoint.lng}
                    </span>
                  </div>
                  <div style={{ flex: 1, width: '100%' }}>
                    <iframe
                      ...
                      allowFullScreen
                      loading="lazy"
                      src={`https://maps.google.com/maps?q=${selectedPinPoint.lat},${selectedPinPoint.lng}&z=16&output=embed`}
                    ></iframe>
                  </div>
                </div>
              </div>
